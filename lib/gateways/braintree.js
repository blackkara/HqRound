...
try{
    ...
    console.error("braintree.json not found or not valid: " + error.message);
    process.exit(1);
}
...
* @param {Boolean} saveDb, if yes saves order and result to DB
    ...
    gateway.transaction.sale(data, function(error, payment){
        ...
        if(!success){
            ...
            if (!(payment.transaction) || payment.transaction.status !== 'authorized') {
                var error = new Error(payment.message)
